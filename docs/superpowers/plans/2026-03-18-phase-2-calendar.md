...
- **`runDeleteByText` center date UTC offset** (from Task 11): `new Date(parsed.start + 'Z')` treats local ISO as UTC for the ±3-day window center. Off by user's UTC offset, but the 3-day window absorbs it. Would fail on narrow windows or large UTC offsets.
- **Duplicate `localISO`/`addMinutesToLocalISO` in test file** (from Task 11): Test file re-implements these helpers for the mock rather than importing from the now-exported production functions. Will silently diverge if production implementations change.
